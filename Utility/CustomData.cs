...
namespace CupAPI.Util {
    ...
    public static class CustomData {
        ...
        public static bool GetObject<T>(int slot, string key, out T value) where T : class {
            value = null;
            if (Data.TryGetValue(slot, out var custom)) {
                if (custom.TryGetValue(key, out var val))
                    try {
        ...
        public static object Get(int slot, string key) {
            if (Data.TryGetValue(slot, out var custom)) {
                if (custom.TryGetValue(key, out var value))
                    return value;
            }
            return null;
        }
