...
public static class EquipRegistries
{
    ...
    public static readonly LinkedRegistry<Charm, IEquipInfo> Charms = new(
        (name, info) => HandlePages(name, info, CharmPages,
            [
    ...
    private static void HandlePages<TEnum>(string name, IEquipInfo info, Dictionary<int, TEnum[]> pagesDict, TEnum[] firstPageValues, TEnum noneValue, int pageAmount) where TEnum : Enum
    {
        if (!pagesDict.ContainsKey(1))
            pagesDict[1] = firstPageValues;

        /*foreach (string icon in info.NormalIcons)
            AssetHelper.CacheAsset<Sprite>(info.BundleName, icon);
